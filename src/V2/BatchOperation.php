...
class BatchOperation extends SerializableWithSyncCoreReference implements IBatchOperation {
  ...
  /**
   * @var ModelInterface|ArrayAccess|JsonSerializable|null $dto
   */
  protected $dto;
  ...
  const REQUEST_FLOW_CREATE = 'flowControllerCreateRequest';
  const REQUEST_POOL_CREATE = 'poolControllerCreateRequest';
  const REQUEST_ENTITY_TYPE_CREATE = 'remoteEntityTypeVersionControllerCreateRequest';
  ...
  public function execute() {
    ...
    $dto = $this->dto;
    if($request_method===self::REQUEST_FLOW_CREATE) {
      $request = $this->core->getClient()->flowControllerCreateRequest($dto);
    }
    elseif($request_method===self::REQUEST_POOL_CREATE) {
      $request = $this->core->getClient()->poolControllerCreateRequest($dto);
    }
    elseif($request_method===self::REQUEST_ENTITY_TYPE_CREATE) {
